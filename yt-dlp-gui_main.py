...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def on_download_button_clicked(self):
        try: 
            ...
            if len(urls) == 0:
                ...
                self.update_status_display(f"Total Download Time: {elapsed_time} seconds")
    ...
    def option_compiler(self): # compile the users options into a string
        ...
        if self.ui.max_download_rate_input.text() != "":
            self.option_output += f"--limit-rate {self.ui.max_download_rate_input.text()} "
        ...
        if self.ui.file_access_retries_input.text() != "":
            self.option_output += f"--file-access-retries {self.ui.file_access_retries_input.text()} "
        if self.ui.fragment_retries_input.text() != "":
            self.option_output += f"--fragment-retries {self.ui.fragment_retries_input.text()} "
        ...
        if self.ui.concurrent_frags_input.text() != "":
            self.option_output += f"--max-concurrent-downloads {self.ui.concurrent_frags_input.text()} "
        ...
        if self.ui.download_sections_input.text() != "":
            self.option_output += f"--download-sections {self.ui.download_sections_input.text()} "
        if self.ui.http_chunk_size_input.text() != "": 
            self.option_output += f"--http-chunk-size {self.ui.http_chunk_size_input.text()} "
